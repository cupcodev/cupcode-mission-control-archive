...
-- Unique constraint for active services per client
create unique index if not exists uq_core_client_services_active 
on core.client_services(client_id, service_id) 
where status = 'active';
...
-- RLS Policies para Client Users
...
-- RLS Policies para Services Catalog
...
-- RLS Policies para Client Services
...
-- Create MC schema if not exists 
create schema if not exists mc;

-- Create mc.workflow_instances table if it doesn't exist
...
begin
  if not exists (
    ...
    where constraint_name = 'fk_mc_wi_client' 
    and table_name = 'workflow_instances'
    and table_schema = 'mc'
  ) then
    alter table mc.workflow_instances
      add constraint fk_mc_wi_client foreign key (client_id)
      references core.clients(id) on delete set null not valid;
  end if;
  
  if not exists (
    select 1 from information_schema.table_constraints 
    where constraint_name = 'fk_mc_wi_service' 
    and table_name = 'workflow_instances'
    and table_schema = 'mc'
  ) then
    alter table mc.workflow_instances
      add constraint fk_mc_wi_service foreign key (service_id)
      references core.services_catalog(id) on delete set null not valid;
  end if;
end $$;
